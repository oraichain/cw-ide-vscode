...
import { Observable, WorkspaceProgress } from './extension';
...
export async function getServer({
  ...
  if (process.arch === 'x64' || process.arch === 'ia32') {
    if (process.platform === 'linux') {
      binaryName = 'cosmwasm-simulate-linux';
    }
    if (process.platform === 'darwin') {
      binaryName = 'cosmwasm-simulate-mac';
    }
 ...
 */
let INSTANCE: lc.BaseLanguageClient | undefined;

/**
 * TODO:
 * Global observable progress
 */
const PROGRESS: Observable<WorkspaceProgress> =
  new Observable<WorkspaceProgress>({ state: 'standby' });
...
async function setupGlobalProgress(client: lc.BaseLanguageClient) {
