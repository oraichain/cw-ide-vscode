...
class CosmJsLatest extends CosmJsAbstract {
   ...
   */
    async handleDeploy(args: { mnemonic: string, wasmBody: string, initInput: any, label?: string | undefined, source?: string | undefined, builder?: string | undefined, gasAmount: { amount: string, denom: string }, fees?: number }) {
        const { mnemonic, wasmBody, initInput, label, source, gasAmount, builder, fees } = args;
        const { current } = window.chainStore;
        ...
        try {
            ...
            const instantiateResult = await client.instantiate(firstAccount.address, codeId, input, label ? label : "smart contract", fees ? fees : 'auto');
